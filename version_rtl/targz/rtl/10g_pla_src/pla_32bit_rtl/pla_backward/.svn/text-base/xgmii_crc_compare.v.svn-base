...
always @( posedge I_312m_clk)
    begin
        S_group_cnt         <=  I_group_cnt      ;
        S_group_cnt_d1      <=  S_group_cnt      ;
...
always @( posedge I_312m_clk)
    begin
        if(I_rst)
            ...
            begin
                if(S_crc_packet_data != I_crc_data_out)
                    begin
                        O_crc_wrong <= 1'b1 ;  
                    end
                else
